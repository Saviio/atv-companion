#!/usr/bin/env npx tsx
/**
 * Wake up (turn on) an Apple TV
 *
 * Usage: npx tsx verify/wake-device.ts --host=<IP> [--port=49152]
 *
...
function parseArgs(): { host: string; port: number } {
  ...
  if (!hostArg) {
    console.error('Usage: npx tsx verify/wake-device.ts --host=<IP> [--port=49152]');
    console.error('\nEnvironment variables:');
...
async function main() {
  ...
  try {
    ...
    await api.pressButton(HidCommand.Wake);

    console.log('Wake command sent successfully!');
    console.log('\nYour Apple TV should now be turning on.');
